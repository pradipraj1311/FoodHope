...
class DonorHomeTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        RankMotivationalBanner(uid: uid, city: userData['city'] ?? 'Unknown', role: 'Donor'),
        Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0), child: SizedBox(width: double.infinity, height: 60, child: ElevatedButton.icon(style: ElevatedButton.styleFrom(backgroundColor: Colors.orange.shade600, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))), onPressed: () => showModalBottomSheet(context: context, isScrollControlled: true, shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))), builder: (context) => PostFoodSheet(userData: userData, uid: uid)), icon: const Icon(Icons.add_circle, size: 28), label: const Text("Post Food Rescue", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold))))),
            ...
            builder: (context, snapshot) {
              ...
              var activeDocs = snapshot.data!.docs.where((doc) {
                ...
                if (data['status'] == 'Available' && data['exactExpiryTime'] != null) {
                  if ((data['exactExpiryTime'] as Timestamp).toDate().isBefore(DateTime.now())) {
                    FirebaseFirestore.instance.collection('donations').doc(doc.id).update({'status': 'Expired'}); return false;
                  }
                }
                return true;
              }).toList();
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (isAccepted) ...[const Divider(height: 20), Container(width: double.infinity, padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.red.shade50, borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.red.shade200)), child: Column(children: [const Text("Give this PIN to the Volunteer:", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold, fontSize: 12)), Text(postData['pickupOtp'] ?? 'Error', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, letterSpacing: 8, color: Colors.red.shade900))])), const SizedBox(height: 8)],
                          if (!isPickedUp && !isAccepted) ...[const SizedBox(height: 15), SizedBox(width: double.infinity, child: OutlinedButton(onPressed: () async { await FirebaseFirestore.instance.collection('donations').doc(post.id).update({'status': 'Cancelled'}); }, style: OutlinedButton.styleFrom(foregroundColor: Colors.red), child: const Text("Cancel Donation")))]
                        ],
                      ),
                    ),
                  );
